...
# define SIZE 10000
using namespace std;
int KMPSearch(string &par,string &ori);
void getNext(string &par,int (&next)[SIZE]);
int main() {
    int n;
    vector<int> rel;
    string par,ori;
    ...
    while(n--) {
        cin >> par >> ori;
        rel.push_back(KMPSearch(par,ori));
    }
    for(auto &cnt:rel) {
        cout << cnt << endl;
    }
...
int KMPSearch(string &par,string &ori) {
    int next[SIZE],i=0,cnt=0,j=0;
    getNext(par,next);
    while(i<ori.size()) {
        while(j<par.size()&&ori[i]==par[j]) {
            i++;
            j++;
        }
        if(j==par.size()) {
...
void getNext(string &par,int (&next)[SIZE]) {
    next[0]=0;
    for(int i=1,j=0;i<par.size();i++) {
        while(par[i]!=par[next[j]]&&j) {
            j=next[j-1];
        }
        if(par[i]==par[next[j]]) {
            next[i]=++j;
